...
public class MidiMessageAnalyzer {
    ...
    public void AnalyzeNote(String receivedDataString){
        ...
        if(mChannel==MidiActivity.keyboardChannel && (pitch>=0 && pitch<88)){ // 입력 신호 채널이 설정한 건반채널일 때
            if(receivedDataString.substring(2,3).equals("9")){ //건반 누른 신호일 때
                PlayNote.noteOff(midiSoundPool, pitch);
                PlayNote.noteOn(midiSoundPool, soundKeys[pitch], pitch, velocity);
                isKeyOn[pitch]=1;
                ...
                if(receivedDataString.substring(6,8).equals("7f")){ // 페달 밟을 경우
                    ...
                    while(!relNote.empty()){
                        ...
                        if(isKeyOn[pit]!=1){ // 해당 건반이 눌려있지 않은 경우 소리 정지
                            isKeyOn[pit]=0;
                            PlayNote.noteOff(midiSoundPool,pit);
                        }
                    }
                    pedalFlag=0;
                }
        ...
        else if(mChannel==MidiActivity.drumChannel){
            // 드럼 연주 기능
        }
